...
<body>
	<form class="box login" action="javascript:void(0)">
		<fieldset class="boxBody">
			...
			<input type="text" tabindex="1" id="username" name="username" placeholder="username or email"
				required maxlength='32' minlength='3'>
			<label>
				Password
				<span class="message"></span>
				<img id="logging" src="${basePath}/static/login/image/logging.gif" style="display: none;">
			</label>
			<input type="password" tabindex="2" id="password" name="password" required placeholder="password"
				maxlength='32' minlength='3'>
		</fieldset>
		<footer>
			<label>
				<input type="checkbox" tabindex="3" id="rememberme">
				Keep me logged in
			</label>
...
<script type="text/javascript">
	...
	function check(username, password) {
		if (username != '' && username.length >= nameMinLength
				&& username.length <= nameMaxLength && password != ''
